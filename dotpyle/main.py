...
@click.command()
@click.option('-u', '--url',required=True, help='Git url of existing repo (GitHub, GitLab, ...)')
@click.option('-p', '--protocol', required=True, type=click.Choice(['git', 'https']),  help='Git protocol')
@click.option('-t', '--token', required=False, help='Git token required if the Git repo is private')
def init(url, protocol, token):
    ...
    process = Popen(['git', 'clone', url, default_config_path], stdout=subprocess.PIPE, universal_newlines=True)
    while True:
